...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"ShowProduct"]) {
        ...
        destination.title123 = [titles objectAtIndex:index];
        NSLog(@"%@",[titles objectAtIndex:index]);

        destination.image = [images objectAtIndex:index];
        destination.price = [costs objectAtIndex:index];

        destination.description123 = [description objectAtIndex: index];

        NSLog(@"%@",[description objectAtIndex:index]);
    }
}
